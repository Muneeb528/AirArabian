...
@section('title', 'AirArabian - Premium Airline Ticketing for UAE, KSA, Umrah & Tours')
                    ...
                    <form action="/" method="GET" class="search-form">
                        <div class="row g-3 align-items-end">
                            <div class="col-md-3">
                                ...
                                <input type="text" name="from" class="search-input" placeholder="Karachi, PAK" value="{{ request('from') }}">
                            </div>
                            <div class="col-md-3">
                                <label class="search-label"><i class="fas fa-plane-arrival me-1"></i>To</label>
                                <input type="text" name="to" class="search-input" placeholder="Dubai, UAE" value="{{ request('to') }}">
                            </div>
                            <div class="col-md-2">
                                <label class="search-label"><i class="fas fa-calendar me-1"></i>Departure</label>
                                <input type="date" name="date" class="search-input" value="{{ request('date') }}" min="{{ date('Y-m-d') }}">
                            </div>
                            <div class="col-md-2">
    ...
    <div class="container">
        ...
        <ul class="nav category-tabs mb-4" id="categoryTabs" role="tablist">
            ...
            <li class="nav-item" role="presentation">
                <button class="category-tab {{ $cat === $category ? 'active' : '' }}"
                        id="tab-{{ $cat }}"
                        data-bs-toggle="tab"
                        data-bs-target="#panel-{{ $cat }}"
                        type="button" role="tab">
                    <i class="fas {{ $cat === 'UAE' ? 'fa-city' : ($cat === 'KSA' ? 'fa-mosque' : ($cat === 'Umrah' ? 'fa-kaaba' : 'fa-globe')) }} me-2"></i>
                    {{ $cat }}
                    <span class="tab-count">{{ $ticketsByCategory[$cat]->count() }}</span>
                </button>
            </li>
            @endforeach
        </ul>
        ...
        <div class="tab-content" id="categoryTabContent">
            @foreach($validCategories as $cat)
            <div class="tab-pane fade {{ $cat === $category ? 'show active' : '' }}" id="panel-{{ $cat }}" role="tabpanel">
               @if($ticketsByCategory[$cat]->isEmpty())
                    <div class="empty-state text-center py-5">
                        <i class="fas fa-plane-slash empty-icon"></i>
                        <h4>No tickets available in {{ $cat }} right now</h4>
                        <p class="text-muted">Check back soon or contact our team for custom packages.</p>
                    </div>
                @else
                <div class="row g-4">
                    @foreach($ticketsByCategory[$cat] as $ticket)
                    <div class="col-lg-4 col-md-6">
                        <div class="ticket-card">
                            <div class="ticket-card-header">
                                <div class="ticket-category-badge">{{ $ticket->category }}</div>
                                <div class="ticket-status-badge {{ $ticket->statusBadgeClass() }}">
                                    {{ ucfirst($ticket->status) }}
                                </div>
                            </div>

                            @if($ticket->image)
                                <img src="{{ Storage::url($ticket->image) }}" class="ticket-image" alt="{{ $ticket->title }}">
                            @else
                            ...
                            <div class="ticket-body">
                                <div class="ticket-route">
                                    <span class="route-city">{{ $ticket->origin }}</span>
                                    <div class="route-arrow">
                                        <i class="fas fa-plane"></i>
                                        <div class="route-line"></div>
                                    </div>
                                    <span class="route-city">{{ $ticket->destination }}</span>
                                </div>

                                <h5 class="ticket-title">{{ $ticket->title }}</h5>

                                @if($ticket->airline)
                                    <p class="ticket-airline"><i class="fas fa-plane-departure me-1"></i>{{ $ticket->airline }}</p>
                                @endif

                                <div class="ticket-meta">
                                    <div class="meta-item">
                                        <i class="fas fa-calendar-alt"></i>
                                        <span>{{ $ticket->departure_date->format('d M Y') }}</span>
                                    </div>
                                    <div class="meta-item">
                                        <i class="fas fa-chair"></i>
                                        <span>{{ $ticket->seats_available }} seats left</span>
                                    </div>
                                </div>
                                ...
                                <div class="ticket-footer">
                                    <div class="ticket-price">
                                        ...
                                        <span class="price-amount">{{ number_format($ticket->price) }}</span>
                                        <span class="price-per">/person</span>
                                    </div>
                                    <a href="https://wa.me/923000000000?text=I'm interested in: {{ urlencode($ticket->title) }}"
        ...
        <div class="vendor-cta-card">
            <div class="row align-items-center">
                ...
                <div class="col-lg-4 text-center text-lg-end mt-4 mt-lg-0">
                    ...
                    <a href="{{ route('vendor.inquiry') }}" class="btn-cta-primary">
...
<section class="routes-section">
    <div class="container">
        ...
        <div class="row g-3">
            ...
            <div class="col-md-3 col-sm-6">
                <a href="{{ route('home') }}?category={{ $route['cat'] }}" class="route-card">
                    <div class="route-icon"><i class="fas {{ $route['icon'] }}"></i></div>
                    <div class="route-info">
                        <div class="route-from-to">{{ $route['from'] }} → {{ $route['to'] }}</div>
                        <div class="route-category">{{ $route['cat'] }}</div>
                        <div class="route-price gold-text">{{ $route['price'] }}</div>
                    </div>
